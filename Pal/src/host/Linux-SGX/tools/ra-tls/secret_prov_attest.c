...
static mbedtls_x509_crt verifier_ca_chain;

static mbedtls_net_context verifier_fd;
static mbedtls_ssl_context ssl;

static mbedtls_pk_context my_ratls_key;
static mbedtls_x509_crt my_ratls_cert;

int secret_provision_start(const char* ca_chain_path, void** out_ssl) {
    int ret;
    ...
    if (!ca_chain_path) {
        ca_chain_path = getenv(SECRET_PROVISION_CA_CHAIN_PATH);
        if (!ca_chain_path) {
            return -EINVAL;
        }
    }

    servers = getenv(SECRET_PROVISION_SERVERS);
    if (!servers) {
        servers = DEFAULT_SERVERS;
    }

    servers = strdup(servers);
    if (!servers) {
        ret = -ENOMEM;
        goto out;
    }
    ...
    ret = secret_provision_read(&ssl, provisioned_secret, provisioned_secret_len);
    if (ret < 0) {
        goto out;
    }
...
int secret_provision_destroy(void) {
    free(provisioned_secret);
    provisioned_secret     = NULL;
    provisioned_secret_len = 0;
    return 0;
...
__attribute__((constructor)) static void secret_provision_constructor(void) {
    ...
    if (e && strlen(e) > 0 && e[0] != '0' && e[0] != 'f' && e[0] != 'F') {
        ...
        int ret = secret_provision_start(/*ca_chain_path=*/NULL, /*out_ssl=*/NULL);
        if (!ret) {
